...
type Open_genType func(driverName, dataSourceName string) (*sql.DB, error)
...
type MoqOpen_genType struct {
	...
	Runtime struct {
		ParameterIndexing struct {
			DriverName     moq.ParamIndexing
			DataSourceName moq.ParamIndexing
		}
	}
}
...
type MoqOpen_genType_params struct{ DriverName, DataSourceName string }

// MoqOpen_genType_paramsKey holds the map key params of the Open_genType type
type MoqOpen_genType_paramsKey struct {
	Params struct{ DriverName, DataSourceName string }
	Hashes struct{ DriverName, DataSourceName hash.Hash }
}
...
type MoqOpen_genType_doFn func(driverName, dataSourceName string)

// MoqOpen_genType_doReturnFn defines the type of function needed when calling
// DoReturnResults for the Open_genType type
type MoqOpen_genType_doReturnFn func(driverName, dataSourceName string) (*sql.DB, error)
...
func NewMoqOpen_genType(scene *moq.Scene, config *moq.Config) *MoqOpen_genType {
	...
	m := &MoqOpen_genType{
		...
		Runtime: struct {
			...
			DriverName:     moq.ParamIndexByValue,
			DataSourceName: moq.ParamIndexByValue,
		}},
	}
	m.Moq.Moq = m
...
func (m *MoqOpen_genType) Mock() Open_genType {
	return func(driverName, dataSourceName string) (*sql.DB, error) {
		...
		return moq.Fn(driverName, dataSourceName)
	}
}

func (m *MoqOpen_genType_mock) Fn(driverName, dataSourceName string) (result1 *sql.DB, result2 error) {
	...
	if result.DoFn != nil {
		result.DoFn(driverName, dataSourceName)
	}
	...
	if result.DoReturnFn != nil {
		result1, result2 = result.DoReturnFn(driverName, dataSourceName)
	}
	return
}

func (m *MoqOpen_genType) OnCall(driverName, dataSourceName string) *MoqOpen_genType_fnRecorder {
...
func (a *MoqOpen_genType_anyParams) DriverName() *MoqOpen_genType_fnRecorder {
	a.Recorder.AnyParams |= 1 << 0
	return a.Recorder
}

func (a *MoqOpen_genType_anyParams) DataSourceName() *MoqOpen_genType_fnRecorder {
...
func (r *MoqOpen_genType_fnRecorder) ReturnResults(result1 *sql.DB, result2 error) *MoqOpen_genType_fnRecorder {
	r.Moq.Scene.T.Helper()
	r.FindResults()
...
func (m *MoqOpen_genType) PrettyParams(params MoqOpen_genType_params) string {
	return fmt.Sprintf("Open_genType(%#v, %#v)", params.DriverName, params.DataSourceName)
}

func (m *MoqOpen_genType) ParamsKey(params MoqOpen_genType_params, anyParams uint64) MoqOpen_genType_paramsKey {
	m.Scene.T.Helper()
	var driverNameUsed string
	var driverNameUsedHash hash.Hash
	if anyParams&(1<<0) == 0 {
		if m.Runtime.ParameterIndexing.DriverName == moq.ParamIndexByValue {
			driverNameUsed = params.DriverName
		} else {
			driverNameUsedHash = hash.DeepHash(params.DriverName)
		}
	}
	var dataSourceNameUsed string
	var dataSourceNameUsedHash hash.Hash
	if anyParams&(1<<1) == 0 {
		if m.Runtime.ParameterIndexing.DataSourceName == moq.ParamIndexByValue {
			dataSourceNameUsed = params.DataSourceName
		} else {
			dataSourceNameUsedHash = hash.DeepHash(params.DataSourceName)
		}
	}
	return MoqOpen_genType_paramsKey{
		Params: struct{ DriverName, DataSourceName string }{
			DriverName:     driverNameUsed,
			DataSourceName: dataSourceNameUsed,
		},
		Hashes: struct{ DriverName, DataSourceName hash.Hash }{
			DriverName:     driverNameUsedHash,
			DataSourceName: dataSourceNameUsedHash,
		},
	}
}
